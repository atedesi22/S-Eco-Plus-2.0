...
<head>
    ...
    <style>
        ...
        /* Masquer la scrollbar pour une expérience plus application mobile */
        .no-scrollbar::-webkit-scrollbar { display: none; }
        .no-scrollbar { -ms-overflow-style: none; scrollbar-width: none; }
    </style>
</head>
<body class="flex flex-col justify-between min-h-screen pb-24 antialiased bg-slate-950 text-slate-100" x-data="{ currentTab: 'home' }">
    ...
    <main class="flex-1 w-full max-w-4xl p-4 mx-auto space-y-6 sm:p-6">
        ...
        <div x-show="currentTab === 'home'" class="space-y-6" x-transition>
            ...
            <!-- Section Soldes Financiers Principaux -->
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div class="relative p-6 overflow-hidden border bg-gradient-to-br from-emerald-500/10 to-slate-900/50 border-emerald-500/20 rounded-2xl">
                    <div class="absolute top-0 right-0 p-4 text-6xl opacity-10 text-emerald-400"><i class="bi bi-wallet2"></i></div>
                    <p class="text-xs font-bold tracking-wider uppercase text-emerald-400">Solde Total Épargné</p>
                    <h2 class="mt-2 text-3xl font-black text-white">{{ number_format($account->balance ?? 50000) }} XAF</h2>
                    <p class="text-[10px] text-slate-500 mt-2">Numéro de compte : {{ $account->account_number ?? 'Non défini' }}</p>
                </div>

                <!-- Solde Réel Disponible -->
                <div class="relative p-6 overflow-hidden border bg-slate-900 border-slate-800 rounded-2xl">
                    <div class="absolute top-0 right-0 p-4 text-6xl opacity-10 text-slate-400"><i class="bi bi-cash-stack"></i></div>
                    <p class="text-xs font-bold tracking-wider uppercase text-slate-400">Disponible pour Retrait</p>
                    @php
                        $balance = $account->balance ?? 50000;
                        $reserve = $account->reserve_fund ?? 1000;
                        $disponible = $balance - $reserve;
                    @endphp
            ...
            <!-- Section Affichage des Sous-Comptes / Tontines Actives -->
            <div class="space-y-3">
                <div class="flex items-center justify-between">
                    <h4 class="text-sm font-bold tracking-wider uppercase text-slate-300">Mes Sous-Comptes & Tontines</h4>
                    <button @click="currentTab = 'tontines'" class="text-xs font-semibold text-emerald-400 hover:underline">Voir tout</button>
                </div>
                        ...
                        <div class="flex items-center justify-between p-4 border bg-slate-900 border-slate-800 rounded-xl">
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-[10px] text-slate-500">Sous-compte N°{{ $account->id ?? 1 }}-{{ $sub->code }}</p>
                                </div>
                            </div>
            ...
            <div class="p-6 border bg-slate-900 border-slate-800 rounded-2xl">
                    ...
                    <div class="space-y-3">
                        @foreach($transactions as $trx)
                            <div class="flex items-center justify-between p-2.5 rounded-xl bg-slate-950/40 border border-slate-800/60">
                                <div class="flex items-center space-x-3">
                                    <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs {{ $trx->type === 'deposit' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400' }}">
                                        <i class="bi {{ $trx->type === 'withdrawal' ? 'bi-arrow-down-left' : 'bi-arrow-up-right' }}"></i>
                                    </div>
                                    <div>
                                        <p class="text-xs font-semibold text-slate-200">{{ $trx->title }}</p>
                                        <p class="text-[10px] text-slate-500">{{ $trx->date }}</p>
                                    </div>
                                </div>
                                <p class="text-xs font-bold {{ $trx->type === 'deposit' ? 'text-emerald-400' : 'text-red-400' }}">
                                    {{ $trx->type === 'withdrawal' ? '+' : '-' }} {{ number_format($trx->amount) }} XAF
                                </p>
                            </div>
                        @endforeach
                    </div>
                @else
                    <div class="py-8 text-xs text-center text-slate-500">
                        ...
                        Aucun mouvement récent sur votre compte principal.
                    </div>
                @endif
            </div>
        </div>
        ...
        <div x-show="currentTab === 'factures'" class="space-y-6" x-transition>
            <div class="p-6 border bg-slate-900 border-slate-800 rounded-2xl">
                ...
                <p class="mt-1 text-xs text-slate-400">Gérez vos transactions financières et réglez vos factures courantes en quelques clics.</p>
            </div>
        ...
        <div x-show="currentTab === 'tontines'" class="space-y-6" x-transition>
            <div class="p-6 border bg-slate-900 border-slate-800 rounded-2xl">
                <h3 class="text-lg font-bold text-white">Mes Cotisations & Tontines Métiers</h3>
                <p class="mt-1 text-xs text-slate-400">Suivez l'état d'avancement de vos différents sous-comptes bloqués et projets.</p>
            </div>

            <!-- Liste Détaillée avec Historique Interne -->
            <div class="space-y-4">
               @foreach($subAccounts as $sub)
                            ...
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <span class="text-[10px] px-2 py-0.5 bg-{{ $sub->color }}-500/10 text-{{ $sub->color }}-400 rounded-full font-semibold">
                                        Objectif : {{ number_format($sub->target) }} XAF
                                    </span>
                                </div>
                            </div>
                            <div class="text-right">
                                <span class="block text-xs text-slate-400">Sous-total</span>
                                <span class="text-base font-bold text-white">{{ number_format($sub->balance) }} XAF</span>
                            </div>
                        </div>

                        <div class="space-y-1">
                            <div class="w-full bg-slate-800 h-1.5 rounded-full overflow-hidden">
                                <div class="bg-{{ $sub->color }}-500 h-full rounded-full" style="width: {{ $sub->progress }}%"></div>
                            </div>
                            <div class="flex justify-between text-[10px] text-slate-500">
                                <span>Progression : {{ $sub->progress }}%</span>
                                <span>Reste : {{ number_format($sub->target - $sub->balance) }} XAF</span>
                            </div>
                        </div>
                    </div>
                @endforeach
            ...
            <!-- Catalogue de Souscription aux nouvelles Tontines -->
            <div class="p-5 space-y-3 border border-dashed border-slate-800 rounded-2xl">
                <h4 class="text-sm font-bold text-white">Découvrir de nouvelles opportunités de Tontine</h4>
                <p class="text-xs text-slate-400">Rejoignez un cycle d'épargne adapté à vos besoins quotidiens.</p>
                <div class="pt-2">
                    <button class="w-full py-2.5 bg-slate-900 border border-slate-700 hover:border-emerald-500 text-xs font-bold rounded-xl text-slate-200 transition">
                        <i class="mr-1 bi bi-plus-circle-dashed text-emerald-400"></i> Souscrire à une nouvelle tontine thématique
                    </button>
                </div>
            </div>
        </div>
    ...
    <nav class="fixed bottom-0 left-0 right-0 z-50 max-w-lg px-2 py-2 mx-auto border-t shadow-2xl bg-slate-900/90 backdrop-blur-md border-slate-800 sm:rounded-t-2xl">
        <div class="flex items-center justify-around">
            ...
            <!-- Accueil -->
            <button @click="currentTab = 'home'" :class="currentTab === 'home' ? 'text-emerald-400 bg-emerald-500/10' : 'text-slate-400 hover:text-slate-200'" class="flex flex-col items-center justify-center flex-1 py-2 transition duration-200 rounded-xl">
                <i class="text-xl bi" :class="currentTab === 'home' ? 'bi-house-door-fill' : 'bi-house-door'"></i>
                <span class="text-[10px] font-medium mt-1">Accueil</span>
            </button>

            <!-- Factures / Transactions -->
            <button @click="currentTab = 'factures'" :class="currentTab === 'factures' ? 'text-emerald-400 bg-emerald-500/10' : 'text-slate-400 hover:text-slate-200'" class="flex flex-col items-center justify-center flex-1 py-2 transition duration-200 rounded-xl">
                <i class="text-xl bi" :class="currentTab === 'factures' ? 'bi-receipt-cutoff' : 'bi-receipt'"></i>
            ...
            <!-- Tontines -->
            <button @click="currentTab = 'tontines'" :class="currentTab === 'tontines' ? 'text-emerald-400 bg-emerald-500/10' : 'text-slate-400 hover:text-slate-200'" class="flex flex-col items-center justify-center flex-1 py-2 transition duration-200 rounded-xl">
                <i class="text-xl bi" :class="currentTab === 'tontines' ? 'bi-pie-chart-fill' : 'bi-pie-chart'"></i>
